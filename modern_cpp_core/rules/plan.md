...
- [x] **`Async Coroutine Timer / Timeout`:**
...
- [x] **`LockFreeStack` (Treiber Stack):**
  - *Upgrade:* Rewrite `treiberStack.cpp`. Achieve truly lock-free performance and strictly zero OS-heap allocation on embedded systems by utilizing a **Pre-allocated Node Pool (FreeList)** combined with **Tagged Indices (Version Counters)**. This design inherently solves both the ABA problem and Memory Reclamation issues without relying on slow `Hazard Pointers` or `std::atomic<std::shared_ptr>`.
- [x] **`ThreadSafeQueue`:**
...
- [x] **`ThreadSafeHashMap`:**
  ...
  - *Upgrade:* Provides two internal bucket strategies for different use cases:
    1. **Linked List:** Useful for scenarios requiring iterator support or where element removals are frequent, using Bucket-Level Locking to protect the list.
    2. **Flat Array / Open Addressing:** Optimized for read-heavy workloads on Embedded systems, maximizing CPU Cache Locality and memory density.
...
**Planned Features:**
- [x] **`AlignedCacheLine`:**
  - *Concept:* False Sharing occurs when two independent variables reside on the same CPU Cache Line, causing threads to step on each other's toes (cache ping-pong). The solution is to use `alignas(std::hardware_destructive_interference_size)` (C++17) for highly contended independent variables (like Atomics, Head/Tail indices) to automatically align them and completely prevent False Sharing, eliminating the need for C++11-era `char` array padding hacks.
- [x] **`std::atomic_ref`:**
...
- [x] **`MonotonicMemoryPool`:**
  - Use C++17/20 `std::pmr::monotonic_buffer_resource` to create ultra-fast allocation zones that don't require individual element deallocation (ideal for short-lived request processing loops). **Must** be initialized with a pre-allocated stack/static buffer and `std::pmr::null_memory_resource()` as the upstream allocator to guarantee zero OS heap allocations during real-time paths.
- [x] **`Thread-safe Initialization (constinit & Block-Scope Statics)`:**
  - *Concept:* Replaces `std::call_once` and `std::once_flag`.
    - For **compile-time static state**, uses C++20 `constinit` to guarantee compile-time initialization, eliminating the "Static Initialization Order Fiasco" without runtime locking overhead.
    - For **runtime Singletons** (e.g., hardware controllers, config parsers), relies on C++ Block-scope static variables (thread-safe lazy initialization, standard since C++11). This natively provides lock-free initialization without the overhead of `std::call_once`.
- [x] **`Atomic Floating-Point (C++20)`:**
...
- [x] **`Relaxed Counters (Telemetry/Metrics)`:**
...
**Planned Features:**
- [x] **ThreadSanitizer (TSan):** Integrate TSan into CMake for testing and CI to detect data races at runtime.
- [x] **Stress Testing:** Write tests simulating heavy contention to intentionally provoke the ABA problem in `LockFreeStack` and verify the logic of `LockFreeQueue`.
